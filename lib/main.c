...
bitmap_t bitmaps[] = {{"Image source", NULL}, {NULL, NULL}};
...
process_task_t process_tasks[] = {{"asm", process_game1_asm}, {NULL, NULL}};
...
void run_processing_task(const gchar *target, int showtime) {
  ...
  for (i = 0; i < SPRITE_COUNT + 1; ++i) {
    ...
    if (bitmaps[i].pixbuf == NULL) {
      bitmaps[i].pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, TRUE, 8, widths[i], heights[i]);
    }
